...
declare const PRECISION: Decimal;
...
declare const MAX_BIN_PER_LIMIT_ORDER: BN;
declare const LIMIT_ORDER_FEE_SHARE: BN;
...
declare function buildBitFlagAndNegateStrategyParameters(x0: BN$1, y0: BN$1, deltaX: BN$1, deltaY: BN$1): {
    bitFlag: number;
    x0: BN$1;
    y0: BN$1;
    deltaX: BN$1;
...
declare function getAmountInBinsBidSide(activeId: BN$1, minDeltaId: BN$1, maxDeltaId: BN$1, deltaY: BN$1, y0: BN$1): AmountIntoBin[];
declare function getAmountInBinsAskSide(activeId: BN$1, binStep: BN$1, minDeltaId: BN$1, maxDeltaId: BN$1, deltaX: BN$1, x0: BN$1): AmountIntoBin[];
declare function toAmountIntoBins(activeId: BN$1, minDeltaId: BN$1, maxDeltaId: BN$1, deltaX: BN$1, deltaY: BN$1, x0: BN$1, y0: BN$1, binStep: BN$1, favorXInActiveBin: boolean): AmountIntoBin[];
interface SimulateWithdrawResult {
    liquidityAndFeeXWithdrawn: BN$1;
    liquidityAndFeeYWithdrawn: BN$1;
    rewardsAmountClaimed: BN$1[];
}
interface SimulateDepositResult {
    totalAmountXDeposited: BN$1;
    totalAmountYDeposited: BN$1;
    actualTotalAmountXDeposited: BN$1;
    actualTotalAmountYDeposited: BN$1;
    actualLiquidityAndFeeXWithdrawn: BN$1;
    actualLiquidityAndFeeYWithdrawn: BN$1;
}
...
declare class RebalancePosition {
    ...
    activeBin: Bin | null;
    currentTimestamp: BN$1;
    constructor(positionAddress: PublicKey, positionData: PositionData, lbPair: LbPair, activeBin: Bin | null, shouldClaimFee: boolean, shouldClaimReward: boolean, currentTimestamp: BN$1);
    static create(params: CreateRebalancePositionParams): Promise<RebalancePosition>;
    _simulateDeposit(binStep: BN$1, tokenXDecimal: BN$1, tokenYDecimal: BN$1, deposits: RebalanceWithDeposit[], simulatedWithdrawResult: SimulateWithdrawResult): {
        result: SimulateDepositResult;
        depositParams: RebalanceAddLiquidityParam[];
    };
    _simulateResize(depositMinBinId: BN$1, depositMaxBinId: BN$1, binStep: BN$1, tokenXDecimal: BN$1, tokenYDecimal: BN$1): void;
    ...
    simulateRebalance(connection: Connection, binStep: BN$1, tokenXDecimal: BN$1, tokenYDecimal: BN$1, withdraws: RebalanceWithWithdraw[], deposits: RebalanceWithDeposit[]): Promise<SimulateRebalanceResp>;
    totalAmounts(): BN$1[];
    totalFeeAmounts(): BN$1[];
    totalRewardAmounts(): BN$1[];
}
interface RebalancePositionBinData {
    ...
    amountX: BN$1;
    amountY: BN$1;
    claimableRewardAmount: BN$1[];
    claimableFeeXAmount: BN$1;
    claimableFeeYAmount: BN$1;
}
...
interface ParsedLimitOrderBinData {
    binId: number;
    empty: boolean;
...
type LimitOrder = IdlAccounts<LbClmm>["limitOrder"];
...
type LimitOrderBinData = IdlTypes<LbClmm>["limitOrderBinData"];
type PlaceLimitOrderParams = IdlTypes<LbClmm>["placeLimitOrderParams"];
interface ParsedLimitOrderWithPubkey {
    publicKey: PublicKey;
    limitOrderData: ParsedLimitOrder;
}
type Oracle = IdlTypes<LbClmm>["oracle"];
...
declare const LIMIT_ORDER_MIN_SIZE = 112;
declare const LIMIT_ORDER_BIN_DATA_SIZE = 32;
...
declare enum PositionPermission {
    ClaimFee = 0
}
declare enum LimitOrderStatus {
    NotFilled = 0,
    PartialFilled = 1,
    Fulfilled = 2
}
...
declare function decodeRewardPerTokenStored(bin: Bin): BN[];
declare function getBinArrayInfoForNonContiguousBinIds(binIds: number[], programId: PublicKey, lbPair: PublicKey): {
    binArrayAccountMetas: AccountMeta[];
...
declare function getExcludedFeeAmount(includedFeeAmount: BN, tradeFeeNumerator: BN): {
    excludedFeeAmount: BN;
    fee: BN;
};
declare function getIncludedFeeAmount(excludedFeeAmount: BN, tradeFeeNumerator: BN): {
    includedFeeAmount: BN;
    fee: BN;
};
declare function splitFee(tradingFee: BN, protocolShare: BN, mmAmountIn: BN, totalAmountIn: BN): {
    fee: BN;
    protocolFee: BN;
};
...
declare function getFeeMode(lbPairState: LbPair, swapForY: boolean): FeeMode;
...
declare function computeBaseFactorFromFeeBps(binStep: BN, feeBps: BN): BN[];
declare function getQPriceFromId(binId: BN, binStep: BN): BN;
declare function getQPriceBaseFactor(binStep: BN): BN;
declare function getC(amount: BN, binStep: number, binId: BN, baseTokenDecimal: number, quoteTokenDecimal: number, minPrice: Decimal, maxPrice: Decimal, k: number): Decimal;
declare function distributeAmountToCompressedBinsByRatio(compressedBinAmount: Map<number, BN>, uncompressedAmount: BN, multiplier: BN, binCapAmount: BN): {
    newCompressedBinAmount: Map<number, BN>;
    loss: BN;
};
declare function getPositionCount(minBinId: BN, maxBinId: BN): BN;
declare function findOptimumDecompressMultiplier(binAmount: Map<number, BN>, tokenDecimal: BN): BN;
declare function compressBinAmount(binAmount: Map<number, BN>, multiplier: BN): {
    compressedBinAmount: Map<number, BN>;
    compressionLoss: BN;
};
declare function generateAmountForBinRange(amount: BN, binStep: number, tokenXDecimal: number, tokenYDecimal: number, minBinId: BN, maxBinId: BN, k: number): Map<number, BN>;
declare function generateBinAmount(amount: BN, binStep: number, binId: BN, tokenXDecimal: number, tokenYDecimal: number, minPrice: Decimal, maxPrice: Decimal, k: number): BN;
 ...
 *   'lowerBinId' and 'upperBinId', representing the range of bin IDs in each chunk.
 */
declare function chunkBinRange(minBinId: number, maxBinId: number, binPerChunk?: number): {
    lowerBinId: number;
    upperBinId: number;
}[];
...
declare function getMultipleMintsExtraAccountMetasForTransferHook(connection: Connection, mintAddressesWithAccountInfo: {
    mintAddress: PublicKey;
    mintAccountInfo: AccountInfo<Buffer>;
}[]): Promise<Map<String, AccountMeta[]>>;
declare function getExtraAccountMetasForTransferHook(connection: Connection, mintAddress: PublicKey, mintAccountInfo: AccountInfo<Buffer>): Promise<AccountMeta[]>;
interface TransferFeeIncludedAmount {
    amount: BN$1;
    transferFee: BN$1;
}
declare function calculateTransferFeeIncludedAmount(transferFeeExcludedAmount: BN$1, mint: Mint, currentEpoch: number): TransferFeeIncludedAmount;
...
declare function getAmountIn(amountOut: BN$1, price: BN$1, swapForY: Boolean, rounding: Rounding): BN$1;
declare function getAmountOut(bin: Bin, inAmount: BN$1, swapForY: boolean): BN$1;
declare function swapExactOutQuoteAtBin(bin: Bin, binStep: number, sParameter: sParameters, vParameter: vParameters, outAmount: BN$1, swapForY: boolean, supportLimitOrder: boolean, feeOnInput: boolean): {
    amountIn: BN$1;
    amountOut: BN$1;
    fee: BN$1;
    protocolFee: BN$1;
};
declare function swapExactInQuoteAtBin(bin: Bin, binStep: number, sParameter: sParameters, vParameter: vParameters, inAmount: BN$1, swapForY: boolean, supportLimitOrder: boolean, feeOnInput: boolean): {
    amountIn: BN$1;
    amountOut: BN$1;
    fee: BN$1;
    protocolFee: BN$1;
};
declare function getLimitOrderLiquidity(bin: Bin, supportLimitOrder: boolean): {
    orderAmountX: BN$1;
    orderAmountY: BN$1;
};
declare function getBinMaxAmountOut(bin: Bin, swapForY: boolean, supportLimitOrder: boolean): BN$1;

/** Result of a TWAP (Time-Weighted Average Price) computation over a time range. */
interface TwapResult<T> {
...
interface IDynamicOracle {
    ...
    getMaxDuration(currentTimestamp: BN$1): BN$1;
    /**
     ...
     */
    getActiveIdByTime(timePoint0: BN$1, timePoint1: BN$1): TwapResult<BN$1> | null;
    /**
     ...
     */
    getActiveId(currentTimestamp: BN$1): TwapResult<BN$1> | null;
    /**
     ...
     */
    getPriceByTime(timePoint0: BN$1, timePoint1: BN$1): TwapResult<Decimal> | null;
    /**
     ...
     */
    getUiPriceByTime(timePoint0: BN$1, timePoint1: BN$1): TwapResult<Decimal> | null;
}
declare class Observation {
    cumulativeActiveBinId: BN$1;
    createdAt: BN$1;
    lastUpdatedAt: BN$1;
...
declare class DynamicOracle implements IDynamicOracle {
    ...
    findCumulativeActiveIdByTimestamp(activeId: BN$1, timestamp: BN$1): BN$1 | null;
    getActiveIdByTime(timePoint0: BN$1, timePoint1: BN$1): TwapResult<BN$1> | null;
    getActiveId(currentTimestamp: BN$1): TwapResult<BN$1> | null;
    getEarliestTimestamp(): BN$1 | null;
    getPriceByTime(timePoint0: BN$1, timePoint1: BN$1): TwapResult<Decimal> | null;
    getUiPriceByTime(timePoint0: BN$1, timePoint1: BN$1): TwapResult<Decimal> | null;
    getMaxDuration(currentTimestamp: BN$1): BN$1;
}
...
declare const unwrapSOLInstruction: (owner: PublicKey, allowOwnerOffCurve?: boolean) => TransactionInstruction;
...
declare function decodeAccount<T extends LbPair | BinArrayBitmapExtension | BinArray | PositionV2 | PresetParameter | LimitOrder | PresetParameter2 | Oracle>(program: Program<LbClmm>, accountName: AccountName, buffer: Buffer): T;
...
declare class DLMM {
     ...
     */
    static getPairPubkeyIfExists(connection: Connection, tokenX: PublicKey, tokenY: PublicKey, binStep: BN, baseFactor: BN, baseFeePowerFactor: BN, concreteFunctionType?: ConcreteFunctionType, collectFeeMode?: CollectFeeMode, opt?: Opt): Promise<PublicKey | null>;
     ...
     */
    static createCustomizablePermissionlessLbPair2(connection: Connection, binStep: BN, tokenX: PublicKey, tokenY: PublicKey, activeId: BN, feeBps: BN, activationType: ActivationType, hasAlphaVault: boolean, creatorKey: PublicKey, activationPoint?: BN, creatorPoolOnOffControl?: boolean, concreteFunctionType?: ConcreteFunctionType, collectFeeMode?: CollectFeeMode, opt?: Opt): Promise<Transaction>;
    /**
     ...
     * @returns A transaction that creates the pair.
     */
    static createCustomizablePermissionlessLbPair(connection: Connection, binStep: BN, tokenX: PublicKey, tokenY: PublicKey, activeId: BN, feeBps: BN, activationType: ActivationType, hasAlphaVault: boolean, creatorKey: PublicKey, activationPoint?: BN, creatorPoolOnOffControl?: boolean, concreteFunctionType?: ConcreteFunctionType, collectFeeMode?: CollectFeeMode, opt?: Opt): Promise<Transaction>;
    /**
     ...
     */
    getLimitOrderByUserAndLbPair(userPubKey: PublicKey): Promise<ParsedLimitOrderWithPubkey[]>;
    /**
     ...
     */
    getLimitOrder(limitOrder: PublicKey): Promise<ParsedLimitOrderWithPubkey>;
    /**
     ...
     */
    quoteCreateLimitOrder({ bins, relativeBin, }: {
        bins: {
            id: number;
        }[];
        relativeBin?: {
     ...
     */
    addLiquidityByWeight2({ positionPubKey, totalXAmount, totalYAmount, xYAmountDistribution, user, slippage, }: TInitializePositionAndAddLiquidityParams, binPerChunk?: number): Promise<Transaction[]>;
    /**
     ...
     */
    enablePositionPermissionlessClaimFee({ position, owner, }: {
        position: PublicKey;
        owner: PublicKey;
    }): Promise<Transaction>;
    /**
     ...
     */
    placeLimitOrder({ owner, payer, sender, limitOrder, params, }: {
        owner: PublicKey;
        payer: PublicKey;
        sender: PublicKey;
     ...
     */
    closeLimitOrderIfEmpty({ limitOrder, owner, rentReceiver, }: {
        limitOrder: PublicKey;
        owner: PublicKey;
        rentReceiver: PublicKey;
    }): Promise<Transaction>;
     ...
     */
    getOracle(): Promise<IDynamicOracle>;
    /**
     ...
     */
    increaseOracleLength(lengthToAdd: BN, funder: PublicKey): Promise<Transaction>;
}
...
declare const limitOrderFilter: () => GetProgramAccountsFilter;
